package org.apache.flink.connector.clickhouse.sink.writer;
...
@PublicEvolving
public abstract class ExtendedAsyncSinkWriter <InputT, RequestEntryT extends Serializable>
    ...
    private final Counter triggeredByMaxBatchSizeCounter;
    private final Counter triggeredByMaxBatchSizeInBytesCounter;
    private final Counter triggeredByMaxTimeInBufferMSCounter;
    private final Histogram actualRecordsPerBatchHistogram;
    private final Histogram actualBytesPerBatchHistogram;
    // TODO: implement actualTimeInBufferHistogram logic
    private final Histogram actualTimeInBufferHistogram;
     ...
     */
    private long bufferedRequestEntriesTotalSizeInBytes;
    ...
    public ExtendedAsyncSinkWriter(
        ...
        this.triggeredByMaxBatchSizeCounter = this.metrics.counter("triggeredByMaxBatchSizeCounter");
        this.triggeredByMaxBatchSizeInBytesCounter = this.metrics.counter("triggeredByMaxBatchSizeInBytesCounter");
        this.triggeredByMaxTimeInBufferMSCounter = this.metrics.counter("triggeredByMaxTimeInBufferMSCounter");

        this.actualRecordsPerBatchHistogram = this.metrics.histogram("actualRecordsPerBatch", new DescriptiveStatisticsHistogram(1000));
        this.actualBytesPerBatchHistogram = this.metrics.histogram("actualBytesPerBatch", new DescriptiveStatisticsHistogram(1000));
        this.actualTimeInBufferHistogram = this.metrics.histogram("actualTimeInBuffer", new DescriptiveStatisticsHistogram(1000));
        ...
        ProcessingTimeService.ProcessingTimeCallback ptc =
                instant -> {
                    ...
                    while (!bufferedRequestEntries.isEmpty()) {
                        // we are counting the number of times we hit maxTimeInBufferMS
                        this.triggeredByMaxTimeInBufferMSCounter.inc();
                        flush();
                    }
                };
    ...
    private void nonBlockingFlush() throws InterruptedException {
        while (!rateLimitingStrategy.shouldBlock(createRequestInfo())
            ...
            // flush in case we reached the limit of bytes
            if (bufferedRequestEntriesTotalSizeInBytes >= maxBatchSizeInBytes)
                this.triggeredByMaxBatchSizeInBytesCounter.inc();
            // flush in case we reached the limit of batch size
            if (bufferedRequestEntries.size() >= getNextBatchSizeLimit())
                this.triggeredByMaxBatchSizeCounter.inc();
            flush();
        }
    }
    ...
    private void flush() throws InterruptedException {
        ...
        // we want to count the reason that we got this flush
        ...
        this.actualBytesPerBatchHistogram.update(bufferedRequestEntriesTotalSizeInBytes);
        this.actualRecordsPerBatchHistogram.update(bufferedRequestEntries.size());

        submitRequestEntries(
                batch, new ExtendedAsyncSinkWriter.AsyncSinkWriterResultHandler(requestTimestamp, batch, requestInfo));
    }
