 ...
 * alive long enough for the upgrade workflow to take a savepoint — a bounded
 * source (e.g. {@code env.fromSequence}) would trigger {@code endOfInput} on
 * the sink and force a flush before the savepoint can be taken.
 *
 * <p>Flink 2.x relocates {@code RichParallelSourceFunction} into the
 * {@code .legacy.} subpackage; otherwise identical to the Flink 1.x version.
 */
