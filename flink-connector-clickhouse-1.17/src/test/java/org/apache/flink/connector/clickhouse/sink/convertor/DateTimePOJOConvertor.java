...
public class DateTimePOJOConvertor extends POJOConvertor<DateTimePOJO> {
    ...
    public void instrument(DataWriter dataWriter, DateTimePOJO input) throws IOException {
        ...
        if (input.dataType.equals(DateTimePOJO.DataType.DATETIME64)) {
            dataWriter.writeDateTime64(input.createdAt.atZone(ZoneOffset.UTC), false, ClickHouseDataType.DateTime64, false, "createdAt", input.precision);
        } else {
            dataWriter.writeDateTime(input.createdAt.atZone(ZoneOffset.UTC), false, ClickHouseDataType.DateTime, false, "createdAt");
        }
